...
(defun eval-app (def app)
  (if (rator? def)
    (eval-op app)
    (if (defun? def)
      (sub-e (defun.formals def) (app.args app)
        (defun.body def))
      app)))

(defun eval-once (defs es)
  (if (atom es)
    '()
    (if (var? (car es))
      (cons (car es) (eval-once defs (cdr es)))
      (if (quote? (car es))
        (cons (car es) (eval-once defs (cdr es)))
        (if (if? (car es))
          (if (quote? (if.Q (car es)))
            (if (quote.value (if.Q (car es)))
              (cons (if.A (car es))
                (eval-once defs (cdr es)))
              (cons (if.E (car es))
                (eval-once defs (cdr es))))
            (cons (if-c (eval-once defs
                          (if.Q (car es)))
                    (if.A (car es))
                    (if.E (car es)))
              (eval-once defs (cdr es))))
          (if (app? (car es))
            (if (quoted-exprs? (app.args (cdr es)))
              (cons (eval-app
                      (lookup (app.name (car es))
                        defs)
                      (car es))
                (eval-once defs (cdr es)))
...
(defun eval-exprs (defs es n)
  ...
  (if (natp n)
    (if (equal n '0)
      ...
      (if (quoted-exprs? es)
        es
        (eval-exprs defs (eval-once defs es)
          (+ '-1 n))))
    es))

(defun eval-expr (defs e n)
  (elem1 (eval-exprs defs (list1 e) n)))
